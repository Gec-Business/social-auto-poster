...
# Fetch all contacts from list 5
print("Fetching contacts from list #5...")
all_contacts = []
limit = 500
offset = 0

while True:
    result = contacts_api.get_contacts_from_list(5, limit=limit, offset=offset)
    batch = result.contacts
    all_contacts.extend(batch)
    print(f"  Fetched {len(all_contacts)} / {result.count}")
    if len(all_contacts) >= result.count:
        break
    offset += limit
...
for contact in all_contacts:
    email_address = contact.get('email') if isinstance(contact, dict) else contact.email
    if not email_address:
        continue
    try:
        smtp_api.send_transac_email(sib_api_v3_sdk.SendSmtpEmail(
            to=[{"email": email_address}],
            sender={"name": "Lia Kereselidze", "email": "lkereselidze@gecbusiness.com"},
            subject="Mindset <> GEC",
            html_content=html_content,
            attachment=[{"content": pdf_base64, "name": "MindsetXGEC offer.pdf"}]
        ))
        sent += 1
        if sent % 50 == 0:
            print(f"  Sent {sent}/{len(all_contacts)}...")
            time.sleep(2)  # avoid rate limiting
    except ApiException as e:
        failed += 1
        print(f"  Failed for {email_address}: {e.status}")
